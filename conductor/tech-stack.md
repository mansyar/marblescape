...
- **Perf gate** (`tests/perf.spec.ts`): Chromium `portrait-phone` project only, CDP `Emulation.setCPUThrottlingRate: 6`, deterministic sandbox stress scene (chain, 5 paced mint drops, mismatch beat, level-1 solve). Asserts p95 ≤ 60 ms mid-stress / ≤ 75 ms after the celebration (crash guards), adaptation engaged (tier ≥ 1, downgrades ≥ 1), ≥ 300 frames, zero rescues, and a confetti burst within the active tier's budget (60 pieces at tier 1 vs the 100-piece full-quality default). Baselines 2026-09-14: stress-tail p95 29–43 ms at 223–231 frames per window; a pinned-full-quality counterfactual measured p95 ≈ 42 ms with 167 frames, so the regression signal comes from the tier/budget assertions, not the generous ceilings. Covered by the `quality-config`, `perf-meter`, `quality`, and `scene` unit suites plus the perf gate; the 4-viewport matrix and both reliability gates are unaffected.
