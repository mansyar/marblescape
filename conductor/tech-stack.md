...
> - **Picture-based level select (2026-09-13, track `level-pictures_20260913`):** no new runtime dependencies, assets, or precache entries — level tiles are camera-matched mini-board snapshots rendered at boot from the existing GLB templates, and the sandbox tile is a token-keyed live snapshot of the child's build; digits/emoji remain only as fallback glyphs. See "Picture-based level select" below.
> - **Gentle lean & flat straights (2026-09-13, track `gentle-lean_20260913`):** no new dependencies, assets, or precache entries — `PHYSICS.gravity` z `2.5 → 1.88` (~6.03° south; the unit test derives and asserts the angle, zero E-W drift) and the straight's manufactured ramp (0.09 rad pitch + lift) is gone: `posePiece` applies yaw only, so live pieces, palette tiles, and level mini-boards all read flush (`slope` removed from `PieceDef`). No re-tuning required — full 4-viewport production matrix green (214 passed / 0 failed; both 20-drop gates and all 9 levels), plus a new lean-flow e2e guard that fails if the lean is removed.
...
- **Snapshots** (`src/render/piece-thumbnails.ts`, `PieceRenderer.templateFor`): one offscreen `WebGLRenderer` pass after templates load renders each piece type (yaw offset applied; every piece sits flush — no pitch or lift, three-quarter camera) to a 160×160 transparent PNG data URL; a failed type is omitted without throwing (word-label fallback) and the offscreen renderer is disposed after the pass.
...
## Picture-based level select

Picture-Based Level Select (track `level-pictures_20260913`, 2026-09-13): **no new dependencies, assets, or precache entries** — tile previews reuse the palette snapshot pipeline and the precache list is unchanged.

- **Board snapshots** (`src/render/board-thumbnails.ts`): `BoardSnapshotSpec` plus `levelSnapshotInput`/`boardSnapshotInput`, `buildBoardSnapshot` (diorama board, posed piece clones, candy-tinted cups, soft gap hints in the in-game placement green), `generateLevelPreviews` (one offscreen capture per board size, fault-tolerant omissions), `createBoardPreview`, and `createBoardPreviewCache` (regenerated only when the serialized board changes). Shared pose/tint helpers (`posePiece`, `tintCup` in `src/render/piece-view.ts`) and the generic `createOffscreenCapture` seam (`src/render/piece-thumbnails.ts`) keep live rendering and previews identical.
- **Integration** (`src/game/game.ts`, `src/main.ts`, `src/ui/level-select.ts`): `Game.createLevelPreviews()` snapshots the nine level starting boards once after `game.start()`; `Game.sandboxPreview()` renders the current sandbox build (the parked build while a level is loaded) behind the token-keyed cache; `createLevelSelect` accepts an optional preview provider, renders `<img>` tiles with `aria-label` `Level N`/`Sandbox`, and falls back to the digit/emoji glyph when a snapshot is unavailable — never a blank tile. Covered by `tests/level-pictures.spec.ts` across the 4-viewport matrix plus the board-thumbnails and level-select unit suites.

## Adaptive quality & performance gate

Adaptive Quality & Performance Gate (track `adaptive-quality_20260913`, 2026-09-14): **no new dependencies, assets, or precache entries** — a frame-pacing meter feeds a three-tier quality governor that caps device pixel ratio and effect budgets, and tiers only change while the game is at rest.

- **Policy core** (`src/render/quality-config.ts`, `src/render/perf-meter.ts`, `src/render/quality.ts`): Tier 0 is today's rendering (`dpr ≤ 2`, 64 sparkles per burst, 120 confetti pieces, shadows + gleam on); Tier 1 caps `dpr ≤ 1.5`, 32 sparkles, 60 confetti pieces; Tier 2 (hard floor) runs `dpr 1`, 16 sparkles, 30 confetti pieces with shadows + gleam off. The meter tracks EMA and rolling p95 frame times; the governor drops a tier after ~3 s of sustained p95 > 25 ms and climbs back only after ~10 s healthy **and** idle (5 s cooldown). Tier-0 budgets are drift-guarded against the legacy constants in unit tests.
- **Integration** (`src/render/scene.ts`, `src/render/sparkles.ts`, `src/ui/confetti.ts`, `src/render/marble-shadow.ts`, `src/render/marble-gleam.ts`, `src/game/game.ts`, `src/main.ts`): tiers apply through the shared framing path (`applyRendererFraming` / `setDprCap`) with pools keeping fixed capacities and effect budgets read at emission time; reduced motion composes with the ladder (smaller budget wins). The `?debug` readout (`src/ui/debug-overlay.ts`) exists only behind the flag.
- **Perf gate** (`tests/perf.spec.ts`): Chromium `portrait-phone` project only, CDP `Emulation.setCPUThrottlingRate` ramped from 6x in +3x steps (3 s each) up to 18x until the governor engages — the fixed 6x rate proved machine-dependent (the GitHub runner rendered the scene too fast to starve: tier stayed 0 with stress p95 ≈ 23 ms), so the ramp keeps the gate meaningful on any machine. Deterministic sandbox stress scene (chain, 5 paced mint drops, mismatch beat, level-1 solve). Asserts p95 ≤ 60 ms mid-stress / ≤ 75 ms after the celebration (crash guards), ≥ 300 frames, zero rescues, and a confetti burst within the active tier's budget (60 pieces at tier 1 vs the 100-piece full-quality default). Adaptation contract: when starvation engaged the governor, downgrades ≥ 1 must be on record; a runner too fast to starve even at the ramp cap must instead keep stress p95 inside the healthy band (≤ 25 ms) — a broken meter/governor would breach it at the cap, so the gate cannot silently go inert. Baselines 2026-09-14: stress-tail p95 29–43 ms at 223–231 frames per window locally (ramp settles at 6x with zero steps); a pinned-full-quality counterfactual measured p95 ≈ 42 ms with 167 frames, so the regression signal comes from the tier/budget assertions, not the generous ceilings. Covered by the `quality-config`, `perf-meter`, `quality`, and `scene` unit suites plus the perf gate; the 4-viewport matrix and both reliability gates are unaffected.
